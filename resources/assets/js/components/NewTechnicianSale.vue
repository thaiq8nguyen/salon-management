<template>
	<div>
		<v-app standalone light>
			<v-layout row wrap>
				<v-flex lg4>
					<v-card>
						<v-card-title>
							<v-date-picker v-model="saleDate" landscape></v-date-picker>
						</v-card-title>
						<v-card-text>
							<v-list>
				...
				<v-flex lg4>
					<div v-if="onTechnician.firstName">
						<div v-if="doesSaleExist()">
							<v-card>
								<v-card>
									<v-card-title primary-title>
										<h2 class = "headline">Add Sale</h2>
									</v-card-title>
									<v-card-text>
										<v-alert info value="true">{{ onTechnician.firstName + '\'s sale is already recorded'}}</v-alert>
									</v-card-text>
								</v-card>
							</v-card>
						</div>
					...
					<div v-else>
						<v-card>
							<v-card-title primary-title>
								<h3 class = "headline">Add Sale</h3>
							</v-card-title>
							<v-card-text>
								<p class = "text-lg-center">Select a technician to begin add sale</p>
							</v-card-text>
							<v-spacer></v-spacer>
							<v-card-text>
    ...
    export default{
        ...
        methods:{
	        ...
	        getTechnicianDetails(firstName){
                this.$axios.get('/api/technician-sale/pay-period/'+ this.payPeriod.id +'/technician/' + firstName).then(response => {
					...
					this.$emit('name', response.data.first_name);
